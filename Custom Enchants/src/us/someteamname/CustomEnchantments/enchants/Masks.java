...
public class Masks implements Listener {
  ...
  public void Reinforce(PlayerMoveEvent event) {
    ...
    if (Enchant.hasEnchant("Reinforce", player.getInventory().getHelmet())) {
      int tier = Enchant.getEnchantTier("Reinforce", player.getInventory().getHelmet());
      if (tier == 2) {
