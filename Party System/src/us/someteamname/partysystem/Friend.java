...
public class Friend {
	public Player t;
	  ...
	  public void perform(MailingAction action, Player p, String t) {
	    Config newFile, file;
	      ...
	      case EIGHT:
			...
			OfflinePlayer tar = Bukkit.getOfflinePlayer(t);
	          ...
	          if (hasFile(tar) && hasFile(p)) {
	            ...
	            if (tar.getName().equals(p.getName())) {
	              msg(p, this.prefix + "&c&oYou cannot befriend yourself... Nice try though");
	              return;
	            } 
	            if (getRequests(tar).contains(p.getName())) {
	              msg(p, this.prefix + "&c&oYou have already sent a request to: " + tar.getName());
	              return;
	            } 
	            if (getFriends(p).contains(tar.getName())) {
	              msg(p, this.prefix + "&c&oYou are already friends with player: " + tar.getName());
	              return;
	            } 
	            ArrayList<String> arrayList1 = new ArrayList<>(getRequests(tar));
	            ArrayList<String> arrayList2 = new ArrayList<>();
	            arrayList1.add(p.getName());
	            arrayList2.add(tar.getName());
	            updateRequests(this.data, arrayList1);
	            updateSentRequests(this.pData, arrayList2);
	            msg(p, this.prefix + "&a&oYou sent a friend request to: " + tar.getName());
	          } else {
	            Config config1 = new Config(tar.getUniqueId().toString());
	            Config config2 = new Config(p.getUniqueId().toString());
	            ArrayList<String> arrayList1 = new ArrayList<>(getRequests(tar));
	            ArrayList<String> arrayList2 = new ArrayList<>();
	            arrayList1.add(p.getName());
	            arrayList2.add(tar.getName());
	        ...
	        if (hasFile(this.t) && hasFile(p)) {
	          ...
	          if (getRequests(this.t).contains(p.getName())) {
	            msg(p, this.prefix + "&c&oYou have already sent a request to: " + this.t.getName());
	            return;
	          } 
	          if (getFriends(p).contains(this.t.getName())) {
	            msg(p, this.prefix + "&c&oYou are already friends with player: " + this.t.getName());
	            return;
	          } 
	          ...
	          updateRequests(this.data, arrayList1);
	          updateSentRequests(this.pData, arrayList2);
	          msg(p, this.prefix + "&a&oYou sent a friend request to: " + this.t.getName());
	          msg(this.t, this.prefix + "&e&oYou recieved a friend request from: " + p.getName());
	          break;
	        } 
	        newFile = new Config(this.t.getUniqueId().toString());
	        file = new Config(p.getUniqueId().toString());
	  ...
	  public void makeFriendship(Player p, Player target) {
	    ...
	    msg(target, this.prefix + "&e&oYou are now friends with: " + p.getName());
	...
	public void perform(FriendAction action, Player p, String playerEdit) {
